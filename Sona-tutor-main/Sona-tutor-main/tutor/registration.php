...
$_POST['first_name'] = trim($_POST['first_name']);
	try {
		...
		$check = 0;
		$e_check = $con->query("SELECT email FROM `users` WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >1 ) {
						...
						$sql = "INSERT INTO `users` (`firstname`,`gender`,`email`,`phone`,`pass`,`type`,`confirmcode`) VALUES ('".$u_fname."','".$u_gender."','".$u_email."','".$u_mobile."','".$u_pass."','".$u_ac."','".$confirmCode."')";
