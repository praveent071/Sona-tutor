...
if (isset($_POST['login'])) {
	if (isset($_POST['email']) && isset($_POST['password'])) {
		...
		$result = $con->query("SELECT * FROM users WHERE (email='$user_login') AND pass='$password_login_md5'");
		...
		if (mysqli_num_rows($result)>0) {
			...
			$online = 'yes';
			$result = $con->query("UPDATE users SET online='$online' WHERE email='$get_user_uname_db'");
...
if(isset($_POST['activate'])){
	if(isset($_POST['actcode'])){
		...
		$result2 = mysql_query("SELECT * FROM users WHERE (email='$user_login') AND confirmCode='$user_acccode'");
