...
<div>
	...
	<div class="nbody" style="margin: 0px 100px; overflow: hidden;">
		<div class="nfeedleft">
				...
				<?php
					$todaydate = date("m/d/Y"); //Month/Day/Year 12/20/2020

					$query = $con->query("SELECT * FROM tutor ORDER BY id DESC");
					while ($row = $query->fetch_assoc()) {
						...
						$post_time = $row['online'];
						$deadline = $row['online'];
						$medium = $row['medium'];
						$medium = str_replace(",", ", ", $medium);

						$query1 = $con->query("SELECT * FROM user1	 WHERE email='$postby_id'");
							...
							<div style="float: right;">';
								if($user!='' && $utype_db=='student'){
									...
									if((strtotime($deadline) - strtotime($todaydate)) < 0){
										...
										$resultpostcheck = $con->query("SELECT * FROM users WHERE t_id='$post_id'");
